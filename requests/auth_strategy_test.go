...
func TestSignedRequestStrategy_CreateAuthHeaders(t *testing.T) {
	...
	headers, err := strategy.CreateAuthHeaders(http.MethodGet, "/idverify/v1/sessions?nonce=abc&timestamp=123", nil)
	assert.NilError(t, err)
...
func TestBuildAuthRequest_NilStrategy(t *testing.T) {
	_, err := BuildAuthRequest(nil, http.MethodGet, "https://api.yoti.com", "/idverify/v1/sessions", nil, nil)
	assert.ErrorContains(t, err, "auth strategy must not be nil")
}
...
func TestBuildAuthRequest_BearerToken(t *testing.T) {
	...
	request, err := BuildAuthRequest(strategy, http.MethodGet, "https://api.yoti.com", "/idverify/v1/sessions", JSONHeaders(), nil)
	assert.NilError(t, err)
	...
	assert.Equal(t, request.URL.Path, "/idverify/v1/sessions")
}
...
func TestBuildAuthRequest_SignedRequest(t *testing.T) {
	...
	request, err := BuildAuthRequest(strategy, http.MethodPost, "https://api.yoti.com", "/idverify/v1/sessions", JSONHeaders(), []byte(`{}`))
	assert.NilError(t, err)
...
func TestBuildAuthRequest_SignedRequest_ExistingQueryParamsPreserved(t *testing.T) {
	key, err := rsa.GenerateKey(rand.Reader, 1024)
	assert.NilError(t, err)
	...
	request, err := BuildAuthRequest(strategy, http.MethodGet, "https://api.yoti.com", "/idverify/v1/supported-documents?includeNonLatin=true", nil, nil)
	assert.NilError(t, err)

	query := request.URL.Query()
	assert.Equal(t, query.Get("includeNonLatin"), "true")
	assert.Check(t, query.Get("nonce") != "")
	assert.Check(t, query.Get("timestamp") != "")
	assert.Equal(t, query.Get("sdkID"), "test-sdk-id")
}
